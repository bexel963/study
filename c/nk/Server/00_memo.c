...
/*
	...
	▶ 인터넷(InterNetwork) : 여러개의 네트워크가 연결된 것

	▶ TCP(Transmission Control Protocol)는 '데이터 전송과정의 컨트롤' 이라는 뜻을 담고 있다.

	▶ TCP/IP 프로토콜 스택

		LINK계층 -> IP계층 -> TCP/UDP계층 -> APPLICATION계층
		
		: TCP/IP 스택은 총 네개의 계층으로 나뉜다. 즉, '인터넷 기반의 효율적인 데이터 정송' 이라는 커다란 하나의 문제를 하나의 덩치 큰 프로토콜로 설계한 것이 아니라,
		  그 문제를 작게 나눠서 계층화하려는 노력이 시도되었고, 그 결과로 탄생한 것이 'TCP/IP 프로토콜 스택'인 것이다.
		  따라서 TCP소켓을 생성해서 데이터를 송수신할 경우에는 다음 네 계층의 도움을 통해서 데이터를 송수신하게 된다.
		
		: 소프트웨어가 존재하기 전에 하드웨어적으로 시스템이 구축되어 있어야 하고, 그러한 물리적 환경을 기반으로 각종 소프트웨어적인 알고리즘을 필요로 한다.
		  즉, 인터넷을 통한 요율적인 데이터의 송수신'이라는 이슈의 해결을 위해서는 아주 많은 분야의 전문가가 필요하며, 이들간의 상호 논의로 만들어진 다양한 약속이 또한
		  필요하다. 따라서 이 문제는 작은 문제로 나눠서 해결하는 것이 효율적이다.
		  결국 문제를 영역별로 나눠서 '인터넷을 통한 효율적인 데이터의 송수신'에 대한 결론을 얻게 되었다.
		  문제를 영역별로 나눠서 해결하다 보니 프로토콜이 여러 개 만들어졌으며, 이들은 계층구조를 통해서 상호간에 관계를 맺게 되었다.

		: 여러 개의 표준을 근거로 설계된 시스템을 가리켜 '개방형 시스템'이라 하며 TCP/IP 프로토폴 스택 역시 개방형 시스템의 하나이다.
		...
		2. IP 계층
			...
			: 물리적인 연결이 형성되었으니, 데이터를 보낼 기본 준비가 되었다. 그런데 복잡하게 연결되어 있는 인터넷을 통한 데이터의 전송을 위해 선행되어야 할 일은 경로의 선택이다.
			  '목적지로 데이터를 전송하기 위해서 중간에 어떤 경로를 거쳐갈 것인가'의 문제를 해결하는 것이 IP계층이고, 이 계층에서 사용하는 프로토콜이 IP이다.
			  IP 자체는 비 연결지향적이며 신뢰할 수 없는 프로토콜이다. 데이터를 전송할 때마다 가져야 할 경로를 선택해 주지만, 그 경로는 일정치 않다.
			  특히 데이터 전송 도중에 경로상에 문제가 발생하면 다른 경로를 선택해 주는데, 이 과정에서 데이터가 손실 되거나 오류가 발생하는 등의 문제가 발생한다고 해서 이를
			  해결해주지 않는다. 즉, 오류발생에 대한 대기가 되어있지 않은 프로토콜이다.

		3. TCP/UDP 계층			

			: 데이터의 전송을 위한 경로의 검색을 IP계층에서 해결해주니, 그 경로를 기준으로 데이터를 전송만하면 된다. TCP와 UDP계층은 이렇듯 IP계층에서 알려준 경로정보를
			  바탕으로 데이터의 실제 송수신을 담당한다. 때문에 이 계층을 가리켜 '전송 계층' 이라 한다.
			  TCP는 신뢰성 있는 데이터의 전송을 담당한다. 그런데 TCP가 데이터를 보낼 때 기반이 되는 프로토콜이 IP이다.(이것이 프로토콜이 스택의 구조로 계층화되어 있는 이유이다.)
			  IP는 오로지 하나의 데이터 패킷(데이터 전송의 기본단위)이 전송되는 과정에만 중심을 두고 설계되었다. 따라서 여러 개의 데이터 패킷을 전송한다 하더라도 각각의 패킷이
			  전송되는 과정은 IP에 의해서 진행되므로 전송의 순서는 물론이거니와 전송 그 자체를 신뢰할 수 없다. 만약에 IP만을 이용해서 데이터를 전송한다면 먼저 정송한 A패킷 보다
			  뒤에 전송한 B패킷이 먼저 도달할 수도 있다. 그리고 이어서 전송한 A, B, C패킷 중에서 A와 C패킷만 전송될 수 있으며, 그나마 C패킷은 송상된 상태로 전송될 수도 있다.
			  반면에 TCP프로토콜이 추가되어 데이터를 송수신하면 다음과 같은 대화를 주고 받게 된다.						

				-> 호스트 A : 1번 패킷 전송
				-> 호스트 B : 1번 패킷 수신 확인 신호 전송

				-> 호스트 A : 2번 패킷 전송
				-> 호스트 B : .....

				-> 호스트 A : 2번 패킷 재전송
				-> 호스트 B : 2번 패킷 수신 확인 신호 전송

			  결론적으로 IP의 상위계층에서 호스트 대 호스트의 데이터 송수신 방식을 약속하는 것이 TCP/UDP이며, TCP는 확인 절차를 걸쳐서 쇤뢰성 없는 IP에 신뢰성을 부여한 프로토콜이다.

		4. APPLICATION 계층

			: 1, 2, 3 계층의 내용은 소켓을 생성하면 데이터 송수신 과정에서 자동으로 처리되는 것들이다. 데이터의 전송경로를 확인하는 과정, 데이터 수신에 대한 응답의 과정이 소켓이라는
			  것 하나에 감춰져 있기 때문이다.
			  소켓이라는 도구를 이용해서 무언가를 만드는 과정에서 프로그램의 성격에 따라 클라이언트와 서버간의 데이터 송수신에 대한 약속들이 정해지기 마련인데, 이를 가리켜 
			  APPLICATION 프로토콜이라 한다.


	▶ int listen(SOCKET s, int backlog)		-> 연결요청 대기상태로의 진입

			  int			-> 성공 시 0, 실패 시 SOCKET_ERROR 반환
			  SOCKET s		-> 연결요청 대기상태에 두고자 하는 소켓의 파일 드스크립터 전달. 이 함수의 인자로 전달된 디스크립터의 소켓이 서버 소켓(리스닝 소켓)이 된다.
			  int backlog	-> 연결요청 대기 큐의 크기정보 전달, 5가 전달되면 큐의 크기가 5가 되어 클라이언트의 연결요청을 5개까지 대기시킬 수 있다.

		: listen 함수가 호출되어야 클라이언트가 연결요청을 할 수 있는 상태가 된다. 즉, listen함수가 호출되어야 클라이언트는 연결요청을 위해서 connect함수를 호출할 수 있다.
		
		: 서버가 '연결요청 대기상태'에 있다는 것은 클라이언트가 연결요청을 했을 때 연결이 수락될 때까지 연결요청 자체를 대기 큐에 대기시킬 수 있는 상태에 있다는 것을 의미한다. 
		
		: 클라이언트의 연결요청도 인터넷을 통해서 흘러 들어오는 일종의 데이터 전송이기 때문에, 이것을 받아들이려면 당연히 소켓이 하나 있어야 한다.
		  서버 소켓의 역할이 바로 이것이다. 즉, 연결요청을 맞이하는, 일종의 문지기 또는 문의 역할을 한다고 볼 수 있다.
		  listen함수가 호출되면, 이렇듯 문지기의 역할을 하는 서버 소켓이 만들어지고, listen함수의 두 번째 인자로 전달되는 정수의 크기에 해당하는 대기실이 만들어 진다.
		  이 대기실을 가리켜 '연결요청 대기 큐'라고 하며 서버 소켓과 연결요청 대기 큐가 완전히 준비되어서 클라이언트의 연결요청을 받아들일 수 있는 상태를 가리켜 
		  '연결요청 대기상태'라 한다.
			  
	▶ SOCKET accept(SOCKET s, struct sockaddr *addr, int *addrlen)		-> 클라이언트의 연결요청 수락
			  
			  SOCKET					-> 성공 시 소켓 핸들, 실패 시 INVALID_SOCKET 반환 
			  SOCKET s					-> 서버 소켓의 파일디스크립터 전달.
			  struct sockaddr *addr		-> 연결요청 한 클라이언트의 주소정보를 담을 변수의 주소 값 전달. 함수 호출이 완료되면 인자로 전달된 주소의 변수에는 클라이언트의 주소정보가 채워진다.
			  int *addrlen				-> 전달된 주소의 변수 크기를 바이트 단위로 전달. 단, 크기정보를 변수에 저장한 다음에 변수의 주소 값을 전달한다.

		: listen함수호출 이후에 클라이언트의 연결요청이 들어왔다면, 들어온 순서대로 연결요청을 수락해야 한다. 연결요청을 수락한다는 것은 클라이언트의 데이터를 주고받을 수 있는
		  상태가 됨을 의미한다.
		  이러한 상태가 되기 위해 문지기 소켓 외에 또 다른 소켓이 필요하다. (데이터를 주고 받으려면 전용 소켓이 있어야 하기 때문...) 그러나 이 소켓은 직접 만들 필요가 없다.
		  accept함수의 호출 결과로 자동으로 소켓이 만들어지고, 이 소켓은 연결요청을 한 클라이언트 소켓과 자동으로 연결까지 된다.
		
		: accept함수는 '연결요청 대기 큐'에서 대기중인 클라이언트의 연결요청을 수락하는 기능의 함수이다. 따라서 accept 함수는 호출성공 시 내부적으로 데이터 입출력에 사용할 소켓을
		  생성하고, 그 소켓의 파일 디스크립터를 반환한다. 중요한 점은 소켓이 자동으로 생성되어, 연결요청을 한 클라이언트 소켓에 연결까지 이뤄진다는 점이다.
		  (대기 큐에 존재하던 연결요청을 하나 꺼내서 새로운 소켓을 생성한 후 연결요청을 완료함.)

	▶ int connect(SOCKET s, const struct sockaddr *name, int namelen)		-> 연결요청

			  int							-> 성공 시 0, 실패 시 SOCKET_ERROR 반환
			  SOCKET s						-> 클라이언트 소켓의 파일 디스크립터 전달. 
			  const struct sockaddr *name	-> 연결요청 할 서버의 주소정보를 담은 변수의 주소 값 전달. 함수호출이 완료되면 인자로 전달된 주소의 변수에는 클라이언트의 주소정보가 채워진다.
											   (연결을 목적으로 하는 서버 소켓의 IP와 PORT정보)
			  int namelen					-> 두 번째 매개변수에 전달된 주소의 변수 크기를 바이트 단위로 전달, 단, 크기정보를 변수에 저장한 다음에 변수의 주소 값을 전달한다.

		: 클라이언트에 의해서 connect함수가 호출되면 다음 둘 중 한가지 상황이 되어야 함수가 반환된다.
			1. 서버에 의해서 연결요청이 접수되었다.
				: '연결요청의 접수'는 서버의 accept함수 호출을 의미하는 것이 아니라 클라이언트의 연결요청 정보가 서버의 연결요청 대기 큐에 등록된 상황을 의미한다.
				  때문에 connect함수가 반환했더라도 당장에 서비스가 이뤄지지 않을 수도 있다.
			2. 네트워크 단절 등 오류상황이 발생해서 연결요청이 중단되었다.

		: 클라이언트 프로그램의 구현순서에는 클라이언트 소켓의 주소할당 과정이 없다.
		  클라이언트 소켓의 주소 할당은 connect함수가 호출될 때 OS(커널)에서 IP는 컴퓨터(호스트)에 할당된 IP로, PORT는 임의로 선택해서 자동으로 할당된다.
		  즉, bind함수를 통해서 소켓에 IP와 PORT를 직접 할당하지 않아도 connect함수호출 시 자동으로 소켓에 IP와 PORT가 할당된다.
		  따라서 클라이언트 프로그램을 구현할 때는 bind함수를 명시적으로 호출할 필요가 없다.
			 
	▶ Iterative 서버의 구현

		: 반복문을 삽입해서 accept함수를 반복 호출하면 된다.
		  accpet함수가 호출된 다음에 입출력 함수인 read, write함수를 호출 하는데 이는 서버소켓을 대상으로 하는것이 아니라, accept함수의 호출 과정에서
		  생성된 소켓을 대상으로 하는 것이다.
		  close함수까지 호출되었다면 한 클라이언트에 대한 서비스가 완료된 것이다. 그 후 이어서 또 다른 클라이언트에게 서비스하기 위해 다시 accept함수부터 호출해야 한다.
		  한 순간에 하나의 클라이언트에게만 서비스를 제공할 수 있는 것이다. 그러나 프로세스와 쓰레드를 이용하면 동시에 둘 이상의 클라이언트에게 서비스를 제공할 수 있게된다.
...
/*
	▶ TCP 소켓에 존재하는 입출력 버퍼
		...
		: send()가 호출되면 출력버퍼라는 곳에 데이터가 전달되어서 상황에 맞게 적절히(한번에 보내든 나눠서 보내든) 데이터를 상대방의 입력버퍼로 전송한다.
		  그러면 상대방은 read()호출을 통해서 입력버퍼에 저장된 데이터를 읽게되는 것이다.

		: '클라이언트의 입력버퍼 크기가 50byte인데, 서버에서 100byte를 전송' 하는 상황은 발생하지 않는다.
		  왜냐하면 TCP에 존재하는 '슬라이딩 윈도우'라는 프로토콜이 데이터의 흐름을 컨트롤하여 서로 대화를 주고받으면서 데이터를 송수신하기 때문에 버퍼가
		  차고 넘쳐서 데이터가 소멸되는 일이 TCP에서는 발생하지 않는다.
		  (입력버퍼 쪽에서 수신 가능한 데이터의 크기를 출력버퍼 쪽에 알려준다.)

		: send()가 반환되는 시점은 상대 호스트로 데이터의 전송이 완료되는 시점이 아닌, 전송할 데이터가 출력버퍼로 이동이 완료되는 시점이다.
		  그러나 TCP의 경우 출력버퍼로 이동된 데이터의 전송을 보장하기 때문에 "send()는 데이터의 전송이 완료되어야 반환이 된다." 라고 표현한다.

	▶ TCP의 내부 동작 원리1 (상대 소켓과의 연결 - Three-way handshaking)
