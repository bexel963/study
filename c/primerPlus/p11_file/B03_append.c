...
int main(void)
{
	FILE* fa, * fs;		// fa는 덧붙일 파일(목적 파일), fs는 소스 파일
	int files = 0;		// 덧붙일 파일 개수
	char file_app[SLEN];	// 덧붙일 파일(목적 파일) 이름
	char file_src[SLEN];	// 소스 파일 이름
	int ch;
	...
	puts("첫 번째 소스 파일의 이름을 입력하시오(끝내려면 빈 라인): ");
...
{
	size_t bytes;
	static char temp[BUFSIZE];	// 한 번만 할당한다.
