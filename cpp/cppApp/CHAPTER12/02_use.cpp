...
int main()
{
	...
	for (i = 0; i < ArSize; i++)
	{
		...
		while (cin && cin.get() != '\n')
			continue;
		/*
			- get(char*, int)의 오래된 버전들은 빈 줄을 읽었을 때, false로 평가하지 않는다.
			  그러한 버전들의 경우에, 빈 줄이 입력되면 문자열의 첫 문자는 널 문자이다.
			  C++시스템이 현재의 C++표준을 따르고 있다면, 빈 줄 입력은 if조건문의 첫 번째 테스트에서 인식된다.
			  반면, 오래된 C++시스템에서는, 빈 줄 입력이 두 번째 테스트에서 인식된다.
		*/
		if (!cin || temp[0] == '\0')
			break;
		else
